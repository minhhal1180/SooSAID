...
| Mobile người dân | Flutter | ⚠️ Mã nguồn hoàn chỉnh cho M01–M04/M06/M09, **chưa biên dịch** (máy phát triển không có Flutter SDK) |
| Crew/Responder PWA | – | ❌ Chưa làm; API `/assignments/*` đã sẵn sàng |
| PostgreSQL + PostGIS | Migration + seed | ✅ SQL hoàn chỉnh, **chưa chạy thật** (máy không có Docker/psql) |

**Kiểm thử:** 70 unit test + 36 acceptance test, tất cả xanh, chạy trên driver
`memory`. Integration test trên PostgreSQL thật **chưa chạy** — bắt buộc trước
khi coi bất kỳ tính năng nào là Done (Rule 15).
...
npm run test:e2e      # 36 acceptance test, map theo TC-xxx trong tests/test_cases.csv
```
...
│   └── mobile/               Flutter app cho người dân
